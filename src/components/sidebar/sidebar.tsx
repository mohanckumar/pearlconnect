...
import SidebarMinifyBtn from './sidebar-minify-btn.js';
				...
				elms.map(function(elm:any) {
					elm.onclick = function(e:any) {
						...
						var loopHeight = setInterval(function() {
							var targetMenu = document.querySelector<any>('#app-sidebar-float-submenu');
							var targetMenuArrow = document.querySelector<any>('#app-sidebar-float-submenu-arrow');
							var targetMenuLine = document.querySelector<any>('#app-sidebar-float-submenu-line');
					...
					elm.onmouseenter = function() {
						...
						if (appElm && appElm.classList.contains('app-sidebar-minified')) {
							...
							if (targetMenuHtml) {
								...
								var sidebar       = document.querySelector<any>('#sidebar');
								...
								if (!document.querySelector('#app-sidebar-float-submenu')) {
									...
									var elm:any = document.getElementById('app-sidebar-float-submenu');
									elm.onmouseover = function() {
										clearTimeout(appSidebarFloatSubmenuTimeout);
									};
									elm.onmouseout = function() {
										appSidebarFloatSubmenuTimeout = setTimeout(() => {
											document.querySelector<any>('#app-sidebar-float-submenu').remove();
										}, 250);
									};
								} else {
									var floatSubmenu = document.querySelector<any>('#app-sidebar-float-submenu');
									var floatSubmenuInnerElm = document.querySelector<any>('#app-sidebar-float-submenu .app-sidebar-float-submenu');
								...
								var targetSubmenuHeight = document.querySelector<any>('#app-sidebar-float-submenu').clientHeight;
								var floatSubmenuElm = document.querySelector<any>('#app-sidebar-float-submenu');
								var floatSubmenuArrowElm = document.querySelector<any>('#app-sidebar-float-submenu-arrow');
								var floatSubmenuLineElm = document.querySelector<any>('#app-sidebar-float-submenu-line');
								...
								document.querySelector<any>('#app-sidebar-float-submenu-line').remove();
								appSidebarFloatSubmenuDom = '';
							}
						}
					}
					elm.onmouseleave = function() {
						var elm = document.querySelector<any>('.app');
